...
from utils.pruning import prune_by_percentile_gradient_perCell, analyze_fisher
...
def main(args_input = None):
    ...
    ########################################################################################################################################
    args.run_name = f"{args.dataset}_bz{args.batch_size}_lr{args.lr}_top{args.top_param}_num{args.times_para}" + f"_{args.run_name}"
    ...
    ######################## Frozen Backbone, Fine-Tuning Head --> Adaptation ###############
    """
        Step 1: Frozen Backbone, Head No Frozen
    """
    ...
    """
        Step 2: Training one or more epoches for head parameters.
    """
    warmup_loss = nn.CrossEntropyLoss().cuda()
    model.train()
    # warm_up_epoches = 4
    warm_up_epoches = args.warmup_epochs
    second_order = hasattr(optimizer, 'is_second_order') and optimizer.is_second_order
    
    # save gradient 
    # fisher_info = {}
    
    for epoch in range(0, warm_up_epoches):
        # ####################################################################################
        for param_group in optimizer.param_groups:
             param_group['lr'] = args.warmup_lr  # 更新优化器中的学习率
            # param_group['lr'] = args.lr  # 更新优化器中的学习率
        for batch_idx, (input, target) in enumerate(loader_train):
            input, target = input.cuda(), target.cuda()
            begin_time = time.time()
            with amp_autocast():
                output = model(input)
                loss = warmup_loss(output, target)
            end_time = time.time()
            print("one iter time:", end_time-begin_time)
            optimizer.zero_grad()
            loss.backward(create_graph=second_order)
            
            # if epoch == warm_up_epoches - 1:
            #     with torch.no_grad():
            #         for name, param in model.named_parameters():
            #             if param.requires_grad and param.grad is not None:
            #                 if name not in fisher_info:
            #                     fisher_info[name] = torch.zeros_like(param)
            #                 fisher_info[name] += (param.grad ** 2)  # 计算 Fisher 信息（梯度平方累积）
    ...
    model.eval()
    eval_metrics = validate(model, loader_eval, validate_loss_fn, args, amp_autocast=amp_autocast)
    print("Warm Up Stage End, Accuracy:{}", eval_metrics["top1"])
    print("*"*50)

    """
        Step 3: Frozen All Params.
    """
    for name, para in model.named_parameters():  
        para.requires_grad = True
    # ##########################################################################################
    if args.init_lr == True:
        for param_group in optimizer.param_groups:
            param_group['lr'] = args.lr  # 更新优化器中的学习率
    ...
    if args.pruning:
        ...
        if args.pruning_method == 'gradient_perCell':
            if args.contrastive:
                ...
                optimizer.zero_grad()
                fisher_information = calculate_gradient(model, loader_grad, optimizer,train_loss_fn, args, amp_autocast=amp_autocast)
                block_ratios = analyze_fisher(fisher_information, top_param=args.top_param)
                mask = prune_by_percentile_gradient_perCell(model, time_para=1, block_ratios=block_ratios)
            else:
                print("2222222222222222222222222222222222222222222222222222222222222")
                optimizer.zero_grad()
                fisher_information = calculate_gradient(model, loader_grad, optimizer,train_loss_fn, args, amp_autocast=amp_autocast)
                block_ratios = analyze_fisher(fisher_information, top_param=args.top_param)
                # block_ratios = generate_schedule('arccos', n_blocks = 12, min_p = 1, max_p = 3)
            ...
            # mask, grad_tensors = prune_by_percentile_gradient_perCell(model, args.times_para)
            
            # top_params = analyze_scl(grad_tensors, top_percent=0.01)
    ...
    if fisher_information:
        print("*****************yg666******************")
        # print("\n Fisher Information Shapes:")
        # for name, tensor in fisher_information.items():
        #     print(f"Layer: {name} | Shape: {tensor.shape} | Mean: {tensor.mean().item()}")
    else:
        print("################# No! ##################")
        print("Fisher Information empty !")
    ...
    import math
    # 假设 model 是你的模型对象
    if args.head_reparam:
        for name, para in model.named_parameters():
            # 检查参数名称中是否包含 'head'，如果包含则需要进行随机初始化
            if 'head' in name:
                para.requires_grad = True
                # 随机初始化 head 部分的权重
                if 'weight' in name:
                    init.kaiming_uniform_(para, a=math.sqrt(5))  # 或者 init.xavier_uniform_(para)，视情况而定
                    # init.kaiming_uniform_(para, mode='fan_out', nonlinearity='relu')
                    # init.xavier_uniform_(para)
                    # init.xavier_uniform_(para)
                elif 'bias' in name:
                    init.zeros_(para)  # 初始化 bias 为零（通常情况下，bias 初始化为零比较好）
            else:
                para.requires_grad = True
    ######################随机初始化############################
    ######################随机初始化############################
    ...
    """
        begin handle keep_ratio
    """
    print("#"*50)
    for name, module in model.named_modules():
        if hasattr(module, "keep_rate"):
            print(name)
            module.keep_rate = args.keep_rate
    print("#"*50)
    ...
    try:
        for epoch in range(start_epoch, num_epochs):
            ...
            # ####################################################################################
            # if (epoch % 30 == 0) & (epoch != 0):  # 每 10 个 epoch 进行 mask 更新
            #     print(f"\n====== Epoch {epoch}: Updating Fisher Information & Mask ======\n")
            #     optimizer.zero_grad()
            #     fisher_information = calculate_gradient(model, loader_grad, optimizer, train_loss_fn, args, amp_autocast=amp_autocast)
            #     block_ratios = analyze_fisher(fisher_information, top_percent=args.top_percent)
            #     mask = prune_by_percentile_gradient_perCell(model, time_para=1, block_ratios=block_ratios)
            # ####################################################################################
            ...
            if best_metric_original < eval_metrics["top1"]:
                ...
                ###################################################################################################
                print("*************best************:"+str(best_metric_original))
                best_path = os.path.join(output_dir, "best_metric_ema_log.txt")
                with open(best_path, "a") as f:
                    f.write(f"Epoch {epoch}: Best Metric EMA = {best_metric_original}\n")
...
def calculate_gradient(model, loader, optimizer, loss_fn, args, amp_autocast=suppress, log_suffix=''):
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        with torch.no_grad():
            for name, param in model.named_parameters():
                if param.requires_grad and param.grad is not None:
                    ...
                    fisher_info[name] += (param.grad ** 2) / N  # 累积 Fisher 信息
        # for name, param in model.named_parameters():
        ...
        #             if name not in fisher_info:
        #                 fisher_info[name] = torch.zeros_like(param)
        #             fisher_info[name] += (param.grad ** 2)  # 计算 Fisher 信息
    ...
    ####################################################################################
    if not fisher_info:
        print(" Warning: Fisher Information is empty! Check gradients.")
        for name, param in model.named_parameters():
            if param.requires_grad:
                print(f"Layer {name}: requires_grad=True, grad={param.grad}")
    
    ###############################return the original parameter state
    for name, param in model.named_parameters():
