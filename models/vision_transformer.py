...
default_cfgs = {
    ...
    'vit_huge_patch14_224_in21k': _cfg(
        url='https://hf-mirror.com/google/vit-huge-patch14-224-in21k/resolve/main/pytorch_model.bin',
        num_classes=21843),
...
class Attention(nn.Module):
    ...
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0., tuning_mode=None, layer_index=None):
        super().__init__()
...
########################################################################################################################
        ...
        self.tuning_layer = [4, 7, 10]
        # self.tuning_layer = [2, 5, 8]
        # self.tuning_layer = [8, 9, 10]
        # self.tuning_layer = [5, 8, 11]
        self.keep_rate = 1.0 #1.0 --> all tokens saved
    ...
    def forward(self, x):
        ...
        """
        left_tokens = N - 1
        # print("self.keep_rate:", self.keep_rate, "self.tuning_layer:", self.tuning_layer)
        if self.keep_rate < 1 and self.layer_index in self.tuning_layer:
            left_tokens = math.ceil(self.keep_rate * (N - 1)) # saved tokens
            cls_attn = attn[:, :, 0, 1:]
...
class Block(nn.Module):
    ...
    def __init__(
            self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0., init_values=None,
            drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, tuning_mode=None, layer_index=None):
        super().__init__()
        self.dim = dim
        ...
        self.attn = Attention(dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop, tuning_mode=tuning_mode, layer_index=layer_index)
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.blocks = nn.Sequential(*[
            ...
            block_fn(
                dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, init_values=init_values,
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, act_layer=act_layer, tuning_mode=tuning_mode_list[i], layer_index=i+1)
    ...
    #     token_masks = []  # 存储每层 Token 遮盖信息
    #     token_selection_info = []  # 存储每层 Token 选择的索引

    #     for i, blk in enumerate(self.blocks):
    #         result = blk(x)
    #         print(f"Layer {i}, blk(x) returned {len(result)} values: {result}")  # 调试信息
    #         x, index, idx, cls_attn, left_tokens = result

    #         if index is not None:
    #             mask = torch.zeros(x.shape[1], dtype=torch.float32, device=x.device)
    #             mask[idx] = 1  # 选中的 Token 设为 1，其他遮盖为 0
    #             token_masks.append(mask.cpu().detach().numpy())
    #             token_selection_info.append({
    #                 "layer": i,
    #                 "selected_tokens": idx.cpu().detach().numpy(),
    ...
    #     return x, token_masks, token_selection_info
