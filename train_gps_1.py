...
def main(args_input = None):
    ...
    print("*"*50)
    print(len(loader_grad), len(loader_train), len(loader_eval))
    print("*"*50)
    # setup loss function
    # if args.jsd_loss:
    ...
    for name, para in model.named_parameters():
        # if 'head' in name or 'norm' in name or 'bias' in name:
        if 'head' in name:
    ...
    """
    # warmup_loss = train_loss_fn
    # warmip_optimizer = torch.optim.Adam(model.parameters(), lr=args.lr/10)
    # warmup_loss = LabelSmoothingCrossEntropy()
    ...
    if args.pruning:
        ...
        if args.pruning_method == 'gradient_perCell':
            if args.contrastive:
                ...
                block_ratios = analyze_fisher(fisher_information, top_percent=args.top_param)
                mask = prune_by_percentile_gradient_perCell(model, time_para=1, block_ratios=block_ratios,data_set=args.dataset)
            else:
        ...
        with torch.no_grad():
            for name, param in model.named_parameters():
                if param.requires_grad and param.grad is not None:
                    ...
                    fisher_info[name] += (param.grad ** 2)  # 累积 Fisher 信息
        # for name, param in model.named_parameters():
